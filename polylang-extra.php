...
class Polylang_Translate_Rewrite_Slugs {
	...
	public function init_action() {
		...
		// Add extra filters for breadcrum navxt plugin
		if (is_plugin_active('breadcrumb-navxt/breadcrumb-navxt.php')) {
			add_filter( 'bcn_breadcrumb_title', array($this, 'custom_bcn_breadcrumb_title'),11,3);
			add_filter( 'bcn_breadcrumb_url',  array($this, 'custom_bcn_breadcrumb_url'), 11,3);
		}
	}
	...
	// Translate title in breadcrumb
	function custom_bcn_breadcrumb_title( $title,$type, $id) {
		$current_lang_postID = pll_get_post($id);
		
		if ($type[0] === 'home') {
			return get_the_title(get_option('page_on_front'));
		}
		
		if($current_lang_postID){
			$post = get_post($current_lang_postID);
	...
	// Translate url in breadcrumb
	function custom_bcn_breadcrumb_url( $url, $type, $id) {
		...
		if($current_lang_postID && !empty($url)){
			return get_permalink($current_lang_postID);
		}
